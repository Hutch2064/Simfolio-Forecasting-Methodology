...
checkout's dependencies, for example:
    ...
    python tests/parity/generate_full_mcmc_sv_source_fixture.py \
      ...
      --output tests/data/full_mcmc_sv_source_parity.json
...
def _load_source_module(source_root: Path, source_script: Path) -> Any:
    ...
    spec = importlib.util.spec_from_file_location("simfolio_pinned_source_gate", source_script)
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("tests/data/full_mcmc_sv_source_parity.json"),
    )
    ...
    # These must be set before importing the source module because source-side
    # numerical imports may initialise Numba cache locations at import time.
    os.environ.setdefault("PYTHONDONTWRITEBYTECODE", "1")
    os.environ.setdefault("NUMBA_CACHE_DIR", str(Path(".numba-cache-source-fixture").resolve()))
